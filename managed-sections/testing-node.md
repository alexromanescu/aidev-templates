...
- The user surface is reachable by a script (Playwright, tRPC client, simulator).
...
- **Unit / integration:** behavior of a function or service in isolation — Vitest in this stack.
- **Simulation:** drives the real subject with deterministic event ordering. Use for concurrency bugs, races, merge algorithms, dual-authority lifecycle hooks. Vitest with a fake clock. See `docs/testing/testing-by-simulation.md` (when present).
- **End-to-end (browser):** Playwright. Reserve for DOM/focus/visual/routing concerns and one smoke test per major journey.
- **Structural (regex / AST):** enforce architectural invariants no behavioral test can prove. See `docs/testing/structural-tests.md` (when present).
...
NOT verification: `tsc --noEmit` passes, "no console errors", "the page loads", "the unit tests pass".

Verification: run the automated workflow test on the real user surface (Playwright MCP for browser, tRPC integration for backend, simulation harness for state logic), check regressions on adjacent features, then declare done.
